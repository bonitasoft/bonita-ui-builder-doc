...
- `BONITA_OPENAPI_URL`: _Since 1.3.15_ URL of the Bonita OpenAPI contract used to power API endpoint autocompletion in the query editor (see xref:development:common-apis-to-use.adoc[Common APIs to use]). By default, it points to the `latest` version of the contract (`{openApiUrl}/latest/openapi.yaml`). To match your Bonita runtime version, set it to the corresponding OpenAPI version. Refer to {openApiUrl} to find the OpenAPI version matching your Bonita version. For example, with Bonita `2025.2`, use OpenAPI version `1.0.6`: `{openApiUrl}/1.0.6/openapi.yaml`.
