...
- `NGINX_TRUSTED_PROXY_CIDR` _Since 1.3.20_: CIDR block of a front proxy (ingress controller, load balancer, another Nginx) placed in front of `bonita-ui-proxy` (default: `127.0.0.1`, an address no proxy connects from). When requests come through such a proxy, the access logs' `$remote_addr` normally shows the proxy's address instead of the end user's, and the real client address is only reachable by parsing the `X-Forwarded-For` field. Setting this variable to the front proxy's CIDR block resolves the end user's address out of that chain into `$remote_addr`, so it appears directly in the access logs. Leave the default if `bonita-ui-proxy` is directly exposed (e.g. Bonita Studio, plain Docker Compose) — nothing changes in that case.
