...
* The Bonita API query editor now provides autocompletion for endpoint paths: when editing a query, the path field suggests the available endpoints filtered by the selected HTTP method, and choosing one automatically fills in the required query parameters. Parameter keys are also autocompleted.
