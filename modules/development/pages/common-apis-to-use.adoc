= Common APIs to use
:page-aliases: ROOT:common-apis-to-use.adoc
...
[TIP]
====
_Since UI Builder 1.3.15_, the Bonita API query editor provides **endpoint autocompletion** to guide you and make building queries easier. When you edit a Bonita API query, the path field suggests the available endpoints (filtered by HTTP method) so you can find the right API without leaving the editor. Selecting an endpoint automatically adds its required query parameters in the *Params* tab, so you never forget a mandatory parameter. Parameter keys are autocompleted as well.

This feature relies on the Bonita OpenAPI contract configured through the `BONITA_OPENAPI_URL` environment variable. See xref:installation:ui-builder-docker-installation.adoc#environment-variables[the Docker configuration guide] for details.
====

Bonita APIs combined with JS objects allow for building all kinds of applications, such as account management dashboard, custom CRM, HR Management tool, invoice processing dashboard, and many more.
...
`q=[query]` is a mandatory parameter. Queries are used to get information about data objects stored in the database. A set of xref:{bonitaVersion}@bonita:data:define-and-deploy-the-bdm.adoc#_queries[default queries], `find` and `find_by`, is defined automatically for each object in the BDM. You can also create custom queries.
...
For information on uploading multiple files, refer to the xref:how-tos:how-to-upload-multiple-documents.adoc[upload multiple files.]
...
[.solution]#Solution#
Ensure that the user has the correct permissions to access the APIs. See more details in this how-to guide: xref:how-tos:how-to-declare-permissions-for-rest-api-request.adoc[How to grant permissions to perform request to Bonita REST API].
--
