...
:page-aliases: ROOT:development-instance-from-studio.adoc,development:development-instance-from-studio.adoc
:description: Learn how to launch Bonita UI Builder from Bonita Studio for development only.

[NOTE]
====
For Bonita Studio Subscription editions only.
====
...
image::development-instance-from-studio/new-project.png[]
...
[WARNING]
====
Using `latest` lets you benefit from the latest features and bug fixes without needing to update your configuration. +
However, it may lead to unexpected issues if a new version is released with breaking changes. +
In addition, it may cause inconsistencies between your local development environment and other environments (e.g., CI, production) that generally use a specific version. +

If you want to have more control over the version used, it is recommended to select a specific version instead of `latest`.
====

If you want to stick to a specific version that is more recent than the Studio, you can type it manually.

image::development-instance-from-studio/custom-version.png[]
...
image::development-instance-from-studio/edit-project-information.png[]
...
* Use the Bonita UI Builder built-in xref:git:version-control-with-git.adoc[Version control with Git].
* Export your application as a JSON file and use the version control on the Bonita project.

UI Builder built-in xref:git:version-control-with-git.adoc[Version control with Git] is recommended if you are
collaborating with others on the same UI Builder application.
This facilitates the conflict resolution process with finer grain artifacts.

image::development-instance-from-studio/git-with-uib.png[]
...
image::development-instance-from-studio/git-with-studio.png[]
...
image::development-instance-from-studio/synchronize-applications.png[]
...
image::development-instance-from-studio/git-mixed.png[]
...
image::development-instance-from-studio/launch.png[]
...
[WARNING]
====
On macOS, be sure to follow the xref:bonita:software-extensibility:bonita-repository-access.adoc#macOS-configuration[specific configuration] to access to Bonita Artifact Repository.
====

[TIP]
====
If encountering issues, check our xref:ROOT:faq.adoc#UIBuilderRestartingFromStudioIssue[FAQ] .
====
...
image::development-instance-from-studio/file-menu.png[]
