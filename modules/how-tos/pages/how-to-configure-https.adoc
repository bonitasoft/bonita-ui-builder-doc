= HTTPS Configuration
:page-aliases: ROOT:how-to-configure-https.adoc
...
[source]
----
server_name ofelia.com;  # server_name directive: Sets the server domain.
...
server {
    listen 80;
    server_name ofelia.com; # should match the server_name declared in https.conf
...
If you deploy Bonita Runtime Tomcat bundle instead of the docker container, make sure you xref:{bonitaVersion}@bonita:identity:ssl.adoc#_tomcat_and_ssl_offloading[activate the RemoteIpValve].
