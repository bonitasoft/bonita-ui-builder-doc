...
:page-aliases: 2024.3@bonita:applications:ui-builder/version-control-with-git/version-control-with-git.adoc
...
* **Branch Persistence:** Your selected Git branch is remembered across browser sessions, so you no longer need to switch back to it each time you reopen the editor.
...
* Each user needs to xref:git-settings.adoc#git-settings-overview[set] their git email and username in their local installation profile.
* From the application settings, xref:connect-git.adoc[connect] to a Git repository.

[NOTE]
====
* Users can xref:git-settings.adoc#git-settings-overview[configure] their git email and username for each application they work on.
...
See xref:git-settings.adoc#disable_auto-commit[Disable Auto-Commit] for details.
====
