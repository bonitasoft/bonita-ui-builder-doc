...
It is a web-based tool launched from a Ofelia custom docker image.
Introduced in the 2024.3 version, it is the new standard used in the Bonita ecosystem for designing interfaces.


=== How to subscribe?
Bonita UI Builder is available by default and for no additional cost in all https://www.ofelia.com/pricing[Scale subscriptions]. It is also available as a payable add-on in https://www.ofelia.com/pricing[Access subscriptions].


=== How to use?
[subs=+macros]
You can easily test a xref:installation:development-instance-from-studio.adoc[Development instance from Studio]. +
Please refer to the xref:getting-started:initiation-manual.adoc[initiation manual] for more general use cases. If you have any other
additional questions, reach out to our {cscRootUrl}[Support team].
...
There should be no change in your logic, continuous integration, and workflows. You can still manage them the way you’re managing them today. Just download the docker image and follow the xref:getting-started:initiation-manual.adoc[initiation manual].
...
* Support and resources: We offer extensive documentation, tutorials, and support to help you make the most of xref:index.adoc[Bonita UI Builder]
* Facilitated transition: We are dedicated to facilitating the transition, with detailed guides on how to use Bonita UI Builder, as well as https://www.ofelia.com/customer-services[professional services support]


=== Can I expect any support from Ofelia?
Yes, you can!
...
Yes, you can! You will need to declare both kinds of applications in the xref:getting-started:builder-declare-interface-in-bonita.adoc[application descriptor].
...
=== Can I use fragments?
While Bonita UI Builder does not natively support creating reusable components for process forms or UI elements (previously known as fragments), there are still xref:development:reuse-code-and-components.adoc[many workarounds] you can use to improve reusability, maintenance, and consistency between your pages and applications.


=== How can I interact with my tasks?
While Bonita UI Builder does not yet support automatic form creation from contract data, you can still easily interact with your tasks, processes, and data. Follow our xref:how-tos:how-to-interact-with-tasks.adoc[dedicated guide] to learn more.
...
[.symptom-description]
image:incorrectAppLink.gif[Application has incorrect Link]

[.cause]#Cause#
If you have customized e.g. the Bonita User Application, you may be using an old version of the layout.

image:issueOldLayout.png[Application with old layout]
...
You may also encounter this issue if you use a xref:{bonitaVersion}@bonita:applications:ui-designer/customize-layouts.adoc[customized layout] instead of the official one. In that case, update your application's layout by re-applying your modifications to the latest layout version that implements the improvements for Application Links.
