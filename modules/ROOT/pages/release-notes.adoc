...
* Fixed an issue on iOS where application canvases were truncated and dropdowns were unreachable because the browser ignored the desktop viewport, leaving the layout too narrow. (BPA-455)
...
* The Bonita API query editor now offers autocompletion for API endpoint paths, sourced from the Bonita OpenAPI contract (fetched at runtime and cached). The path field suggests available endpoints filtered by HTTP method, selecting an endpoint automatically populates its required query parameters in the *Params* tab, and parameter keys are autocompleted as well. See xref:installation:ui-builder-docker-installation.adoc#environment-variables[the installation guide] to configure the retrieval of the API contract, or xref:development:common-apis-to-use.adoc[Common APIs to use] for more details.
